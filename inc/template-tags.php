...
if ( ! function_exists( 'dro_pizza_entry_footer' ) ) :
	...
	function dro_pizza_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'dro-pizza' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'dro-pizza' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
